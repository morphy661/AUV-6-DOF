"""Run the hash-locked one-shot V3 sensor display-policy benchmark."""
...
DEFAULT_PROTOCOL = (
    REPOSITORY_ROOT
    / "docs"
    / "six_dof_sensor_fault_observer_protocol_v3.json"
)
...
def validate_protocol(protocol, protocol_path, scenarios):
    ...
    if int(configuration["mission_count"]) != (
...
def main():
    ...
    summary = summarize_sensor_fault_observer_benchmark_v3(rows)
    payload = {
        "benchmark": "six_dof_sensor_fault_observer_v3",
    ...
    json_path = output_dir / "sensor_fault_observer_v3_summary.json"
    csv_path = output_dir / "sensor_fault_observer_v3_missions.csv"
    ...
    for key, value in summary.items():
        if key not in ("per_scenario", "acceptance_checks", "display_policy"):
            print(f"  {key}: {value}")
